...
# Must have at least 5 alternatives in same county
# =============================================================================
...
def count_alternatives_in_county(pin):
    """Count how many other providers with embeddings are in same county."""
    county = pin_to_county_state.get(pin, '')
    same_county_pins = county_state_to_pins.get(county, [])
    # Count those with embeddings, excluding the pin itself
    return len([p for p in same_county_pins if p in pin_to_emb and p != pin])

# Get WHOS labeled hospitals with at least 5 alternatives
whos_pins = [pin for pin, label in pin_to_label.items() 
             if label == 'WHOS' and pin in pin_to_idx and count_alternatives_in_county(pin) >= 5]
print(f"WHOS hospitals with >=5 alternatives: {len(whos_pins)}")

# Get unlabeled hospitals with at least 5 alternatives
unlabeled_pins = [pin for pin in all_pins_list 
                  if pin not in pin_to_label and count_alternatives_in_county(pin) >= 5]
print(f"Unlabeled hospitals with >=5 alternatives: {len(unlabeled_pins)}")
