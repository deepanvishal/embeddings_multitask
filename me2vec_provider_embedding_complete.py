...
print("\nBuilding procedure co-occurrence graph...")
n_providers, n_codes = proc_matrix_filtered.shape
cooccurrence = defaultdict(float)

for provider_idx in range(n_providers):
    provider_codes = proc_matrix_filtered[provider_idx].nonzero()[1]
    provider_claims = proc_matrix_filtered[provider_idx].data
    
    code_claim_dict = dict(zip(provider_codes, provider_claims))
    
    for i, code1 in enumerate(provider_codes):
        for code2 in provider_codes[i+1:]:
            edge = tuple(sorted([int(code1), int(code2)]))
            weight = min(code_claim_dict[code1], code_claim_dict[code2])
            cooccurrence[edge] += weight

print(f"Total co-occurrence edges: {len(cooccurrence)}")

G = nx.Graph()
for (code1, code2), weight in cooccurrence.items():
...
def biased_random_walk(G, start_node, walk_length, p, q):
    ...
    walk.append(np.random.choice(list(G.neighbors(start_node))))
...
for provider_idx in range(len(all_pins)):
    ...
    for code_idx, claim_count in zip(provider_codes, provider_claims):
        ...
        if code_id in code_embeddings:
            weighted_embs.append(code_embeddings[code_id] * claim_count)
            total_weight += claim_count
    
    if weighted_embs and total_weight > 0:
        provider_emb = np.sum(weighted_embs, axis=0) / total_weight
    else:
        provider_emb = np.zeros(CODE_EMBEDDING_DIM)
    
    provider_init_embeddings.append(provider_emb)

provider_init_embeddings = np.array(provider_init_embeddings)
...
for epoch in range(EPOCHS):
    ...
    batch_indices = [labeled_indices[i:i+BATCH_SIZE] 
                     for i in range(0, len(labeled_indices), BATCH_SIZE)]
    ...
    for batch_idx_list in batch_indices:
        ...
        for provider_idx in batch_idx_list:
            ...
            specialty_label = torch.LongTensor([specialty_to_id[pin_to_label[pin]]]).to(device)
...
print("\nGenerating embeddings for all providers...")
with torch.no_grad():
    for provider_idx in range(len(all_pins)):
        provider_codes = proc_matrix_filtered[provider_idx].nonzero()[1]
        
        if len(provider_codes) == 0:
            final_embeddings.append(provider_init_embeddings[provider_idx])
            continue
        ...
        final_embeddings.append(updated_emb.cpu().numpy())
...
for i in range(final_embeddings.shape[1]):
    embedding_df[f'emb_{i}'] = final_embeddings[:, i]

specialty_column = []
for pin in all_pins:
    if pin in pin_to_label:
        specialty_column.append(pin_to_label[pin])
    else:
        specialty_column.append('UNLABELED')
embedding_df['specialty'] = specialty_column
...
embedding_df_head = embedding_df.head(10)
print("\nFirst 10 rows preview:")
print(embedding_df_head[['PIN', 'emb_0', 'emb_1', 'emb_2', 'specialty']])
