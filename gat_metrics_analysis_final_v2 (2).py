...
# CRITICAL: Load actual procedure codes mapping AND claim counts
print("Loading procedure codes mapping and claim counts...")
procedure_df = pd.read_parquet('procedure_df.parquet', columns=['PIN', 'code', 'claims'])
all_procedure_codes_ordered = sorted(procedure_df['code'].unique())
print(f"  Total unique procedure codes: {len(all_procedure_codes_ordered)}")
print(f"  Procedure matrix columns: {proc_matrix.shape[1]}")
if proc_matrix.shape[1] != len(all_procedure_codes_ordered):
    print(f"  WARNING: Matrix columns ({proc_matrix.shape[1]}) != unique codes ({len(all_procedure_codes_ordered)})")

# Create mapping: column_index -> actual_code
column_idx_to_code = {idx: code for idx, code in enumerate(all_procedure_codes_ordered)}
print(f"  Sample mapping: column 0 -> '{column_idx_to_code[0]}', column 100 -> '{column_idx_to_code.get(100, 'N/A')}'")

# Create PIN -> {code: claim_count} lookup for ACTUAL raw claim counts
print("Building PIN to claim counts lookup...")
pin_code_claims_lookup = {
    pin: dict(zip(group['code'].values, group['claims'].values))
    for pin, group in procedure_df.groupby('PIN')
}
print(f"  Built lookup for {len(pin_code_claims_lookup):,} providers")
...
if os.path.exists(ATTENTION_CACHE_FILE):
    ...
    # Validate: Check if codes are actual CPT codes (strings) or column indices (integers)
    sample_pin = list(attention_cache.keys())[0]
    sample_codes = attention_cache[sample_pin]['codes']
    if len(sample_codes) > 0:
        first_code = sample_codes[0]
        if isinstance(first_code, int):
            print("\n  ⚠️  WARNING: Cache contains column indices (integers), not actual codes!")
            print("  ⚠️  This was created with old code. Delete and re-run to get actual CPT codes.")
            print(f"  ⚠️  Delete: rm {ATTENTION_CACHE_FILE}\n")
        else:
            print(f"  ✓ Validation: codes are actual CPT codes (e.g., '{first_code}')")
else:
        ...
        for batch_start in tqdm(range(0, len(all_unique_pins), BATCH_SIZE_ATTENTION), desc="Extracting"):
            ...
            for pin in batch_pins:
                ...
                for code_idx, count in zip(code_indices, code_counts):
                    column_position = all_specialty_codes[code_idx]  # Column index in original matrix
                    actual_code = column_idx_to_code.get(column_position)  # Map to actual CPT code
                    
                    if actual_code is None:
                        continue  # Skip if mapping not found
                    
                    if column_position in code_to_idx_map:
                        code_list.append(actual_code)  # Store ACTUAL code
                        code_embs_list.append(code_embeddings_array[code_to_idx_map[column_position]])
    ...
    # Validation: Check sample codes
    sample_pins = list(attention_cache.keys())[:3]
    for pin in sample_pins:
        if len(attention_cache[pin]['codes']) > 0:
            print(f"  Sample provider {pin}: codes = {attention_cache[pin]['codes'][:3]}")
            break
...
for idx in tqdm(range(len(pairs_df)), desc="Computing"):
    ...
    # NEW METRIC: Alternative's ACTUAL claims on primary's top 5 codes
    alternative_claims_on_primary_top5 = 0
    if len(p_top5_codes) > 0 and a_pin in pin_code_claims_lookup:
        a_code_to_claims = pin_code_claims_lookup[a_pin]
        for code in p_top5_codes:
            alternative_claims_on_primary_top5 += a_code_to_claims.get(code, 0)
    
    # Convert to int (actual claim counts are integers)
    alternative_claims_on_primary_top5 = int(alternative_claims_on_primary_top5)
...
print("    = Sum of alternative's ACTUAL RAW CLAIMS on primary's top 5 attention codes")
print("    = Integer value (e.g., 330 claims, not matrix values)")
