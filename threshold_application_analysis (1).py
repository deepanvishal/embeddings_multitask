...
PRIMARY_SPECIALTY_FILTER = 'WHO'  # Only analyze this specialty as primary
# =============================================================================

print("\n" + "="*80)
print("THRESHOLD APPLICATION & DISTRIBUTION ANALYSIS")
print(f"Similarity Threshold: {SIMILARITY_THRESHOLD}")
print(f"Primary Provider Filter: {PRIMARY_SPECIALTY_FILTER} specialty only")
print("="*80)
...
print(f"\nTotal pairs before filtering: {len(pairs_df):,}")
...
print(f"Unique {PRIMARY_SPECIALTY_FILTER} providers: {pairs_df['pin_a'].nunique():,}")
...
print("\n" + "="*80)
print(f"PER-PROVIDER ANALYSIS ({PRIMARY_SPECIALTY_FILTER} ONLY)")
print("="*80)

good_pairs_df = pairs_df[pairs_df['is_good_pair']].copy()

print(f"\nCounting good alternatives per {PRIMARY_SPECIALTY_FILTER} provider...")

# Only count for pin_a (which are all WHO after filtering)
provider_good_count = good_pairs_df.groupby('pin_a').size()

# Get all WHO providers that appeared in pairs
...
print("\n" + "="*80)
print(f"DISTRIBUTION ANALYSIS - EXACT COUNTS ({PRIMARY_SPECIALTY_FILTER})")
print("="*80)
...
print("\n" + "="*80)
print(f"DISTRIBUTION ANALYSIS - BUCKETED ({PRIMARY_SPECIALTY_FILTER})")
print("="*80)
...
print("\n" + "="*80)
print(f"SUMMARY STATISTICS ({PRIMARY_SPECIALTY_FILTER} PROVIDERS)")
print("="*80)

print(f"\nOverall statistics:")
print(f"  Total {PRIMARY_SPECIALTY_FILTER} providers analyzed: {len(provider_alt_df):,}")
...
print(f"\nCoverage:")
print(f"  {PRIMARY_SPECIALTY_FILTER} providers with 0 alternatives: {providers_with_zero:,} ({providers_with_zero/len(provider_alt_df)*100:.1f}%)")
print(f"  {PRIMARY_SPECIALTY_FILTER} providers with ≥1 alternatives: {providers_with_any:,} ({providers_with_any/len(provider_alt_df)*100:.1f}%)")
print(f"  {PRIMARY_SPECIALTY_FILTER} providers with ≥5 alternatives: {(provider_alt_df['n_good_alternatives'] >= 5).sum():,}")
print(f"  {PRIMARY_SPECIALTY_FILTER} providers with ≥10 alternatives: {(provider_alt_df['n_good_alternatives'] >= 10).sum():,}")
...
axes[0, 2].set_ylabel(f'Number of {PRIMARY_SPECIALTY_FILTER} Providers')
axes[0, 2].set_title(f'Distribution: Alternatives per {PRIMARY_SPECIALTY_FILTER} Provider')
axes[0, 2].legend()
...
axes[1, 0].set_ylabel(f'Number of {PRIMARY_SPECIALTY_FILTER} Providers')
axes[1, 0].set_title(f'Bucketed Distribution ({PRIMARY_SPECIALTY_FILTER})')
...
axes[1, 1].set_ylabel(f'Cumulative % of {PRIMARY_SPECIALTY_FILTER} Providers')
axes[1, 1].set_title(f'Cumulative Distribution ({PRIMARY_SPECIALTY_FILTER})')
...
axes[1, 2].set_ylabel(f'Number of {PRIMARY_SPECIALTY_FILTER} Providers')
axes[1, 2].set_title(f'Top 30 Most Common Counts ({PRIMARY_SPECIALTY_FILTER})')
...
summary_stats = {
    'threshold': SIMILARITY_THRESHOLD,
    'primary_specialty_filter': PRIMARY_SPECIALTY_FILTER,
    'total_pairs': len(pairs_df),
    'good_pairs': n_good,
    'bad_pairs': n_bad,
    'total_who_providers': len(provider_alt_df),
