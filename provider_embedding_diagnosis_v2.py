...
Usage: python provider_embedding_diagnosis.py
"""
...
def analyze_provider_procedures(provider_A_idx, provider_B_idx, provider_C_idx):
    ...
    print(f"{'='*60}")
    print(f"Provider A: {all_pins[provider_A_idx]} - {len(A_dict)} procedures")
    print(f"Provider B: {all_pins[provider_B_idx]} - {len(B_dict)} procedures")
    print(f"Provider C: {all_pins[provider_C_idx]} - {len(C_dict)} procedures")
    
    # Get specialties if available
    if all_pins[provider_A_idx] in pin_to_label:
        print(f"  A Specialty: {pin_to_label[all_pins[provider_A_idx]]}")
    if all_pins[provider_B_idx] in pin_to_label:
        print(f"  B Specialty: {pin_to_label[all_pins[provider_B_idx]]}")
    if all_pins[provider_C_idx] in pin_to_label:
        print(f"  C Specialty: {pin_to_label[all_pins[provider_C_idx]]}")
...
def analyze_graph_positions(proc_df):
    ...
    for idx, row in proc_df.iterrows():
        ...
        if len(neighbors) > 1:
            ...
            # Sample for performance if too many neighbors
...
def analyze_embedding_contributions(provider_A_idx, provider_B_idx, provider_C_idx, proc_df):
    ...
    if len(ab_only_codes) > 0:
        ...
        if np.any(ab_contribution_A) and np.any(provider_A_emb):
    ...
    if len(ac_only_codes) > 0:
        ...
        if np.any(ac_contribution_A) and np.any(provider_A_emb):
    ...
    if len(abc_all_codes) > 0:
        ...
        if np.any(abc_contribution_A) and np.any(provider_A_emb):
...
def find_similar_discrepancy_cases(num_examples=5):
    """
    Find other provider triplets with similar discrepancy patterns
    """
    print(f"\n{'='*80}")
    print("FINDING SIMILAR DISCREPANCY CASES")
    print(f"{'='*80}")
    
    discrepancies = []
    
    # Sample providers to check
    sample_size = min(100, len(all_pins))
    sampled_indices = np.random.choice(len(all_pins), sample_size, replace=False)
    
    print(f"Scanning {sample_size} providers for discrepancy patterns...")
    
    for i, idx_a in enumerate(sampled_indices):
        if i % 20 == 0:
            print(f"  Progress: {i}/{sample_size}")
        
        # Get procedures for provider A
        a_procs = set(proc_matrix_filtered[idx_a].nonzero()[1])
        if len(a_procs) < 50:  # Skip providers with too few procedures
            continue
        
        # Find providers with different overlap patterns
        for idx_b in sampled_indices:
            if idx_b == idx_a:
                continue
            
            b_procs = set(proc_matrix_filtered[idx_b].nonzero()[1])
            overlap_ab = len(a_procs & b_procs)
            
            if overlap_ab < 100:  # Need substantial overlap
                continue
            
            for idx_c in sampled_indices:
                if idx_c in [idx_a, idx_b]:
                    continue
                
                c_procs = set(proc_matrix_filtered[idx_c].nonzero()[1])
                overlap_ac = len(a_procs & c_procs)
                
                # Look for cases where AC overlap > AB overlap
                if overlap_ac > overlap_ab * 1.2:  # At least 20% more overlap
                    # Check similarities
                    sim_ab = 1 - cosine(final_embeddings[idx_a], final_embeddings[idx_b])
                    sim_ac = 1 - cosine(final_embeddings[idx_a], final_embeddings[idx_c])
                    
                    # Look for discrepancy
                    if sim_ab > sim_ac:  # Discrepancy found
                        discrepancies.append({
                            ...
                            'sim_ab': sim_ab,
                            'sim_ac': sim_ac,
                            'discrepancy': sim_ab - sim_ac,
                            'overlap_ratio': overlap_ac / overlap_ab
                        })
    ...
    if len(discrepancies) > 0:
        ...
        print(f"\nFound {len(discrepancies)} cases with similar discrepancy pattern")
        print(f"\nTop {min(num_examples, len(discrepancies))} examples:")
        print("-" * 80)
        print("A_idx  B_idx  C_idx  |  AB_overlap  AC_overlap  |  AB_sim  AC_sim  |  Discrepancy")
        print("-" * 80)
        
        for i, case in enumerate(discrepancies[:num_examples]):
            print(f"{case['idx_a']:5d}  {case['idx_b']:5d}  {case['idx_c']:5d}  |  "
                  f"{case['overlap_ab']:10d}  {case['overlap_ac']:10d}  |  "
                  f"{case['sim_ab']:6.3f}  {case['sim_ac']:6.3f}  |  "
                  f"{case['discrepancy']:+.3f}")
    else:
        print("No similar discrepancy cases found in sample")
...
if __name__ == "__main__":
    ...
    missing_files = []
    if provider_init_embeddings is None:
        missing_files.append("provider_init_embeddings.npy")
    if len(code_embeddings) == 0:
        missing_files.append("code_embeddings_dict.npy")
    if cooccurrence_matrix is None:
        missing_files.append("cooccurrence_matrix.npz")
    
    if missing_files:
        print(f"\n⚠ Warning: Some analyses will be skipped due to missing files:")
        for f in missing_files:
            print(f"  - {f}")
        print("\nTo enable full analysis, please re-run me2vec with the additional save statements shown above.")
        proceed = input("\nProceed with limited analysis? (y/n): ")
        if proceed.lower() != 'y':
            exit()
    
    # Primary method: PIN input
    print("\nEnter Provider PINs for analysis")
    print("(These are the actual provider IDs from your data)")
    print("-" * 40)
    
    # Get PINs with validation
    while True:
        pin_A = input("Enter PIN for Provider A: ").strip()
        if pin_A not in pin_to_idx:
            print(f"  Error: PIN '{pin_A}' not found in data")
            print(f"  Available PINs example: {list(pin_to_idx.keys())[:5]}")
            continue
        break
    
    while True:
        pin_B = input("Enter PIN for Provider B: ").strip()
        if pin_B not in pin_to_idx:
            print(f"  Error: PIN '{pin_B}' not found in data")
            continue
        if pin_B == pin_A:
            print(f"  Error: Provider B must be different from Provider A")
            continue
        break
    
    while True:
        pin_C = input("Enter PIN for Provider C: ").strip()
        if pin_C not in pin_to_idx:
            print(f"  Error: PIN '{pin_C}' not found in data")
            continue
        if pin_C in [pin_A, pin_B]:
            print(f"  Error: Provider C must be different from A and B")
            continue
        break
    
    # Convert PINs to indices
    provider_A_idx = pin_to_idx[pin_A]
    provider_B_idx = pin_to_idx[pin_B]
    provider_C_idx = pin_to_idx[pin_C]
    
    print(f"\nSelected providers:")
    print(f"  Provider A: {pin_A} (index: {provider_A_idx})")
    print(f"  Provider B: {pin_B} (index: {provider_B_idx})")
    print(f"  Provider C: {pin_C} (index: {provider_C_idx})")
    ...
    if save_results.lower() == 'y':
        filename = f"diagnosis_{pin_A}_{pin_B}_{pin_C}.csv"
        proc_df.to_csv(filename, index=False)
        print(f"Saved detailed procedure analysis to {filename}")
