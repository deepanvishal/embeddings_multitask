...
print("\n" + "="*80)
print("SPECIALTY-LEVEL ANALYSIS (OVERALL)")
print("="*80)
...
# Analysis by label
label_summary = provider_alt_df.groupby('label').agg({
    'has_alternatives': ['sum', lambda x: (~x).sum(), 'count']
}).reset_index()

label_summary.columns = ['label', 'providers_with_alternatives', 'providers_without_alternatives', 'total_providers']

label_summary['pct_without_alternatives'] = (
    label_summary['providers_without_alternatives'] / label_summary['total_providers'] * 100
)

label_summary = label_summary.sort_values('providers_without_alternatives', ascending=False)

print(f"\nPROVIDER LABEL DISTRIBUTION:")
print(f"{'Label':<30s} {'With Alt':<10s} {'Without Alt':<12s} {'Total':<8s} {'% Without':<10s}")
print("-" * 80)
...
# Analysis by specialty category
specialty_summary = provider_alt_df.groupby('specialty').agg({
    'has_alternatives': ['sum', lambda x: (~x).sum(), 'count']
}).reset_index()

specialty_summary.columns = ['specialty', 'providers_with_alternatives', 'providers_without_alternatives', 'total_providers']

specialty_summary['pct_without_alternatives'] = (
    specialty_summary['providers_without_alternatives'] / specialty_summary['total_providers'] * 100
)

specialty_summary = specialty_summary.sort_values('providers_without_alternatives', ascending=False)

print(f"\nSPECIALTY CATEGORY DISTRIBUTION:")
print(f"{'Specialty':<30s} {'With Alt':<10s} {'Without Alt':<12s} {'Total':<8s} {'% Without':<10s}")
print("-" * 80)

for _, row in specialty_summary.head(30).iterrows():
    print(f"{row['specialty']:<30s} "
          f"{int(row['providers_with_alternatives']):<10d} "
          f"{int(row['providers_without_alternatives']):<12d} "
          f"{int(row['total_providers']):<8d} "
          f"{row['pct_without_alternatives']:>6.1f}%")
...
print("\n" + "="*80)
print("COUNTY-LEVEL SPECIALTY ANALYSIS")
print("="*80)

# County + Label breakdown
county_label_summary = provider_alt_df.groupby(['county_state', 'label']).agg({
    'has_alternatives': ['sum', lambda x: (~x).sum(), 'count']
}).reset_index()

county_label_summary.columns = ['county_state', 'label', 'providers_with_alternatives', 
                                 'providers_without_alternatives', 'total_providers']

county_label_summary['pct_without_alternatives'] = (
    county_label_summary['providers_without_alternatives'] / county_label_summary['total_providers'] * 100
)

county_label_summary[['county_nm', 'state']] = county_label_summary['county_state'].str.split('|', expand=True)

print(f"\nCounty + Label combinations: {len(county_label_summary)}")
print(f"\nTop 20 County+Label combinations by providers without alternatives:")
print(f"{'County':<25s} {'State':<6s} {'Label':<25s} {'With Alt':<10s} {'Without Alt':<12s} {'Total':<8s}")
print("-" * 100)

county_label_top = county_label_summary.sort_values('providers_without_alternatives', ascending=False)
for _, row in county_label_top.head(20).iterrows():
    print(f"{row['county_nm']:<25s} {row['state']:<6s} {row['label']:<25s} "
          f"{int(row['providers_with_alternatives']):<10d} "
          f"{int(row['providers_without_alternatives']):<12d} "
          f"{int(row['total_providers']):<8d}")

# County + Specialty breakdown
county_specialty_summary = provider_alt_df.groupby(['county_state', 'specialty']).agg({
    'has_alternatives': ['sum', lambda x: (~x).sum(), 'count']
}).reset_index()

county_specialty_summary.columns = ['county_state', 'specialty', 'providers_with_alternatives', 
                                     'providers_without_alternatives', 'total_providers']

county_specialty_summary['pct_without_alternatives'] = (
    county_specialty_summary['providers_without_alternatives'] / county_specialty_summary['total_providers'] * 100
)

county_specialty_summary[['county_nm', 'state']] = county_specialty_summary['county_state'].str.split('|', expand=True)

print(f"\nCounty + Specialty combinations: {len(county_specialty_summary)}")
print(f"\nTop 20 County+Specialty combinations by providers without alternatives:")
print(f"{'County':<25s} {'State':<6s} {'Specialty':<25s} {'With Alt':<10s} {'Without Alt':<12s} {'Total':<8s}")
print("-" * 100)

county_specialty_top = county_specialty_summary.sort_values('providers_without_alternatives', ascending=False)
for _, row in county_specialty_top.head(20).iterrows():
    print(f"{row['county_nm']:<25s} {row['state']:<6s} {row['specialty']:<25s} "
          f"{int(row['providers_with_alternatives']):<10d} "
          f"{int(row['providers_without_alternatives']):<12d} "
          f"{int(row['total_providers']):<8d}")
...
top_10_labels = label_summary.head(10)
x = range(len(top_10_labels))
axes[2, 0].bar(x, top_10_labels['providers_without_alternatives'], 
               color='red', alpha=0.7, label='Without Alternatives')
axes[2, 0].bar(x, top_10_labels['providers_with_alternatives'], 
               bottom=top_10_labels['providers_without_alternatives'],
               color='green', alpha=0.7, label='With Alternatives')
axes[2, 0].set_xticks(x)
axes[2, 0].set_xticklabels(top_10_labels['label'], rotation=45, ha='right', fontsize=8)
axes[2, 0].set_ylabel('Number of Providers')
axes[2, 0].set_title('Top 10 Provider Labels by Providers Without Alternatives')
axes[2, 0].legend()
axes[2, 0].grid(True, alpha=0.3, axis='y')

# 6. Specialty category distribution
top_10_specialties = specialty_summary.head(10)
x = range(len(top_10_specialties))
axes[2, 1].bar(x, top_10_specialties['providers_without_alternatives'], 
               color='red', alpha=0.7, label='Without Alternatives')
axes[2, 1].bar(x, top_10_specialties['providers_with_alternatives'], 
               bottom=top_10_specialties['providers_without_alternatives'],
               color='green', alpha=0.7, label='With Alternatives')
axes[2, 1].set_xticks(x)
axes[2, 1].set_xticklabels(top_10_specialties['specialty'], rotation=45, ha='right', fontsize=8)
axes[2, 1].set_ylabel('Number of Providers')
axes[2, 1].set_title('Top 10 Specialty Categories by Providers Without Alternatives')
axes[2, 1].legend()
...
label_summary.to_csv(f'label_level_alternatives_{PRIMARY_SPECIALTY_FILTER}_threshold_{SIMILARITY_THRESHOLD}.csv', 
                     index=False)
print(f"✓ Saved: label_level_alternatives_{PRIMARY_SPECIALTY_FILTER}_threshold_{SIMILARITY_THRESHOLD}.csv")

specialty_summary.to_csv(f'specialty_level_alternatives_{PRIMARY_SPECIALTY_FILTER}_threshold_{SIMILARITY_THRESHOLD}.csv', 
                         index=False)
print(f"✓ Saved: specialty_level_alternatives_{PRIMARY_SPECIALTY_FILTER}_threshold_{SIMILARITY_THRESHOLD}.csv")

county_label_summary.to_csv(f'county_label_alternatives_{PRIMARY_SPECIALTY_FILTER}_threshold_{SIMILARITY_THRESHOLD}.csv', 
                             index=False)
print(f"✓ Saved: county_label_alternatives_{PRIMARY_SPECIALTY_FILTER}_threshold_{SIMILARITY_THRESHOLD}.csv")

county_specialty_summary.to_csv(f'county_specialty_alternatives_{PRIMARY_SPECIALTY_FILTER}_threshold_{SIMILARITY_THRESHOLD}.csv', 
                                 index=False)
print(f"✓ Saved: county_specialty_alternatives_{PRIMARY_SPECIALTY_FILTER}_threshold_{SIMILARITY_THRESHOLD}.csv")
