...
print("Strategy: Sample counties weighted by pair count potential")

eligible_counties = [cs for cs, pins in county_state_to_pins.items() 
                     if len(pins) >= 2 and cs != '|']
print(f"Eligible counties (≥2 providers): {len(eligible_counties)}")

county_pair_potential = {}
for county_state in eligible_counties:
    pins = county_state_to_pins[county_state]
    n = len(pins)
    potential = n * (n - 1) // 2
    county_pair_potential[county_state] = potential

total_potential = sum(county_pair_potential.values())
print(f"Total potential pairs across all counties: {total_potential:,}")

pairs_set = set()
county_sampling_attempts = 0
max_attempts = N_PAIRS * 100

while len(pairs_set) < N_PAIRS and county_sampling_attempts < max_attempts:
    county_state = np.random.choice(eligible_counties)
    county_pins = county_state_to_pins[county_state]
    
    if len(county_pins) < 2:
        continue
    
    pin_a = np.random.choice(county_pins)
    pin_b = np.random.choice(county_pins)
    
    if pin_a != pin_b:
        idx_a = all_pins.tolist().index(pin_a) if pin_a in all_pins else None
        idx_b = all_pins.tolist().index(pin_b) if pin_b in all_pins else None
        
        if idx_a is not None and idx_b is not None:
            pair = tuple(sorted([idx_a, idx_b]))
            pairs_set.add(pair)
    
    county_sampling_attempts += 1
    
    if len(pairs_set) % 100 == 0 and len(pairs_set) > 0:
        print(f"  Generated {len(pairs_set):,} pairs...", end='\r')

print(f"\n✓ Generated {len(pairs_set):,} unique same-county pairs")

if len(pairs_set) < N_PAIRS:
    print(f"  Warning: Only generated {len(pairs_set):,} pairs (requested {N_PAIRS:,})")
    print(f"  This may indicate limited provider pairs within counties")
