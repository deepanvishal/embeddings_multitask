...
# Get WHOS by specialty category code with at least 5 alternatives
whos_pins = [pin for pin in all_pins_list 
             if pin_to_specialty.get(pin) == 'WHOS' and count_alternatives_in_county(pin) >= 5]
print(f"WHOS with >=5 alternatives: {len(whos_pins)}")

# Sort by procedure count, take top 20%, sample 10
...
top_whos = whos_with_procedures[:max(1, len(whos_with_procedures) // 5)]

random.seed(42)
if len(top_whos) >= 10:
    selected = random.sample(top_whos, 10)
else:
    selected = top_whos
